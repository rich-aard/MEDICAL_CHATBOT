pipeline {
    ...
    environment {
        ...
        IMAGE_TAG         = "${BUILD_NUMBER}"        // Automatically increments with every Jenkins run
    }
    ...
    stages {
        ...
        stage('Workspace Cleanup') {
            steps {
                echo 'Cleaning up intermediate local images from the host system engine...'
    ...
    post {
        success {
            echo "Build #${BUILD_NUMBER} compiled, tested, and pushed to Docker Hub successfully!"
        }
        failure {
            echo "Pipeline failed at build #${BUILD_NUMBER}. Dive into the Console Output above to trace errors."
        }
    }
}